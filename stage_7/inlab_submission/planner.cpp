...
{
	return ((userCode == MAGIC_CODE) ? ADMIN_LEVEL : (userCode == EXIT_CODE) ? EXIT_LEVEL
												 : (userCode > 0)			 ? USER_LEVEL
																			 : INVALID_LEVEL);
}
		...
		else
		{
			...
			case ADMIN_ENTER_JOURNEY_INFO:
				...
				for (int i = 0; i < numStops; i++)
				{
					...
					{
						if (daysOfWeek % 10 == 1)
							stnTrainInfo->setDayOfWeek((j + (dayCount - 1)) % 7);
						else if (daysOfWeek % 10 == 0)
							stnTrainInfo->resetDayOfWeek(j + (dayCount - 1) % 7);
						else
						{
	...
	do
	{
		...
		else
		{
			...
			case USER_ENTER_DELETE_REVIEW:
				...
				{ // option == USER_ENTER_REVIEW or option == USER_DELETE_REVIEW
			...
			case USER_FIND_JOURNEYS:
				...
				else
				{
					...
					while (currJCodeReview != nullptr)
					{
						...
						cout << "Source: " << srcStnName << ",   Destination: " << destStnName << endl
							 << endl;
			...
			case USER_FIND_ALL_JOURNEYS_INFO:
				...
				if (sStnName != "")
				{
					...
					if (!readFrmFile)
					{
						cout << "Chosen station: " << sStnName << endl;
					}
				}
				else
				{
...
void Planner::printStationInfo(listOfObjects<TrainInfoPerStation *> *stnInfoList)
{
	...
	while (currList != nullptr)
	{
		...
		if (currInfo != nullptr)
		{
			...
			cout << endl;
			cout << "Arrival time: " << currInfo->arrTime << endl;
			cout << "Departure time: " << currInfo->depTime << endl;
			cout << endl
				 << endl;
		}
