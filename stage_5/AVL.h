...
private:
  ...
  bool checkImbalance(TreeNode* node);// to check the balance of the nodes
  void updateNodes(TreeNode* node);// updates the heights
  bool deleteNode(TreeNode* node);// deletes the node
public:
  // Constructors
