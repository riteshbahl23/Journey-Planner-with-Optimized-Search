...
Entry:: Entry() {
    for(int i=0;i<32;i++) st_1.name[i]='\0';
    TOMB_STONE=false;
}

void Entry::set( Station st_1,Station st_2 ){
    strcpy(this->st_1.name,st_1.name);
    //cout << this->st_1.name<<" "<< st_1.name <<endl;
    strcpy(this->st_2.name,st_2.name);
    //cout << this->st_2.name<<" "<< st_2.name << endl;
    this-> st_1.time = st_1.time;
    this-> st_2.time = st_2.time;
    TOMB_STONE= false;
}
void Entry::operator=(Entry a){
...
bool Dictionary::put(struct Entry e){
    ...
    cout << e.st_1.name <<" "<<e.st_2.name<<endl;
    cout <<"end put"<<endl;*/
...
void Dictionary::Printout(){
    for (int i=0;i<N;i++){
        if (A[i].st_1.name[0] == '\0') continue;
        cout << i << ". \n";
        cout << "STATION-1-NAME:" << A[i].st_1.name<<endl
        << "START TIME"<<A[i].st_1.time << endl
        << "STATION-2-NAME:" << A[i].st_2.name <<endl
        << "END TIME"<<A[i].st_2.time<<endl;
    }
}